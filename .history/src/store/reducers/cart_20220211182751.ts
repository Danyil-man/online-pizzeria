    ...
    switch(action.type){
        case ADD_PIZZA_CART:{
            ...
            const newItems = {
                ...
                [action.pizzaObj.id]: {
                    items: currentPizzas,
                    totalCartPrice: currentPizzas.reduce((sum, obj:PizzaCartType)=>
                    obj.price + sum,0)
                }
            }
            
           
            const totalCount = Object.keys(newItems).reduce( (sum:number, key) =>
            //@ts-ignore
            newItems[key].items.length + sum, 0)

            const price = Object.keys(newItems).reduce( (sum:number, key) =>
            //@ts-ignore
        ...
        case MINUS_CART_ITEM:{
            //@ts-ignore
            const newItems = state.items[action.id].items.slice(1)
                return{
                    ...state,
                    items: {
                        ...state.items,
